...
export const DashboardUserCard = ({ data, index }) => {
  ...
  const [isUserRoleUpdated, setIsUserRoledUpdated] = useState(false)
  const createdAt = moment(new Date(data.createdAt)).format("MMMM Do YYYY");

  const updateUserRole =(userId, role) =>{
    setIsUserRoledUpdated(false)
  ...
  return(
    <motion.div key={index} className='relative w-full rounded-md flex items-center justify-between py-4 bg-lightOverlay cursor-pointer hover:bg-card hover:shadow-md'>
      ...
      <div className='w-275 min-w-[160px] text-center flex items-center justify-center gap-6 relative '>
        <p className='text-base text-textColor text_center'>{data.role}</p>
        {
          data._id !== user?.user._id &&(
            <motion.p whileTap={{scale : 0.75}} className='text-[10px] font-semibold text-textColor px-1 bg-purple-200 rounded-sm hover:shadow-md' onClick={() => setIsUserRoledUpdated(true)}>
          ...
          className='absolute z-10 top-6 right-4 p-4 flex items-start flex-col gap-4 bg-white shadow-xl rounded-md'>
            ...
            <div className='flex items-center gap-4'>
              <motion.button whileTap={{scale : 0.75}} className='outline-none border-none text-sm px-4 py1 rounded-md bg-blue-200 text-black hover:shadow-md' onClick={()=> updateUserRole(data._id, data.role === "admin" ? "member" : "admin")}>
               ...
               <motion.button whileTap={{scale : 0.75}} className='outline-none border-none text-sm px-4 py1 rounded-md bg-gray-200 text-black hover:shadow-md' onClick={() => setIsUserRoledUpdated(false)}>
                No
              </motion.button>
            </div>
          </motion.div>
